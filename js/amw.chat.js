...
setInterval("send_recieve_messages()", 5000);
...
function get_users(status) {//get a list of online users and send current user's status
    ...
    $.ajax({
        url: 'http://alexmakeswebsites.com/wp-content/plugins/amw-chat/connect.php',
        type: "POST",
        data: "status=" + status,
...
function send_recieve_messages() {
    ...
    //alert(last_message);
    $.ajax({
        url: 'http://alexmakeswebsites.com/wp-content/plugins/amw-chat/communication.php',
        type: "POST",
